...
`docker compose up` gives you a working product: `house-db` creates the
...
By default these talk to the `house-db` container through
...
```bash
DATABASE_URL=postgres://user:pass@host:5432/wrenhouse npm run demo:status
```
