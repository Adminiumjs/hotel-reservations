...
function harness(opts: { conns?: unknown[]; routes?: Record<string, unknown> } = {}) {
  ...
  const fetchImpl = vi.fn(async (input: unknown) => {
    ...
    if (path === "/api/v1/bootstrap") payload = { data: { csrfToken: "csrf-abc" } };
      ...
      payload = { connections: opts.conns ?? [{ id: "conn-1", name: "Hotel DB", timezone: "Europe/Lisbon", currency: "EUR" }] };
...
const SCHEMA_OK = {
  "/api/v1/connections/conn-1/schema": {
    model: {
      tables: [
        { name: "room_types", columns: [{ name: "id" }, { name: "name" }] },
        { name: "stays", columns: [{ name: "ref" }, { name: "first_name" }] },
      ],
    },
  },
};
...
describe("discovery", () => {
  ...
  it("says the connection is PAUSED rather than missing when it is", async () => {
     ...
     */
    const { fetchImpl } = harness({ conns: [{ id: "a", name: "Hotel DB", disabled: true }] });
    const port = sessionPort({ tableOfRef: TABLE_OF_REF, fetchImpl });
    await expect(port.config()).rejects.toMatchObject({ code: "CONNECTION_PAUSED" });
    await expect(port.config()).rejects.toThrow(/Hotel DB/);
  });
  ...
  it("points at a PAUSED connection when the schema does not match", async () => {
    ...
    const { fetchImpl } = harness({
      ...
      conns: [
        ...
        { id: "conn-2", name: "c_hotel_reservations", disabled: true },
      ],
    });
    const port = sessionPort({ tableOfRef: TABLE_OF_REF, fetchImpl });
    const err = await port
      .assertRefs({ payments: ["stay_ref"] })
    ...
    expect(message).toContain("c_hotel_reservations is paused in Adminium");
